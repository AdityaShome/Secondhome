      ...
      try {
        ...
        if (account?.provider === "google" || account?.provider === "facebook") {
          ...
          if (!existingUser) {
            ...
            // Update image if it's missing or if Google has a newer image
            const googleImage = user.image || (profile as any)?.picture
            if (googleImage) {
              try {
                // Always update image from Google to ensure it's current
                await User.updateOne(
                  { _id: existingUser._id },
                  { $set: { image: googleImage, emailVerified: new Date() } }
                )
                console.log("✅ Updated OAuth user image:", existingUser._id)
                // Update existingUser object for immediate use
                existingUser.image = googleImage
              } catch (error: any) {
                console.error("❌ Error updating OAuth user image:", error)
                // Don't fail sign-in if image update fails, just log it
                // The image will still be available from user.image
              }
            }
          }
          ...
          // Use the image from database (which was just updated) or fallback to Google image
          user.image = existingUser.image || user.image || (profile as any)?.picture || null
          
          console.log("✅ OAuth sign-in successful for user:", user.id, "Image:", user.image ? "present" : "missing")
          return true
        }
